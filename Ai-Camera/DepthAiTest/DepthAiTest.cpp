...
#include <iostream>
#define consoleOnly 0

int main() {
    std::string nnPath = "best.rvc2.tar.xz";
    if (consoleOnly) {
        YoloTracker camera(nnPath);
        //std::shared_ptr<std::vector<FruitPos>> fruits = camera.getFruitsPtr();
        while (camera.pipeline.isRunning()) {
            
            std::vector fruits = camera.checkFruit();
            if (fruits.size() > 0) {
                std::cout << "x:" << fruits[fruits.size()-1].x << ", y:" << fruits[fruits.size() - 1].y << ", z:" << fruits[fruits.size() - 1].z << " ripeness:" << fruits[fruits.size() - 1].ripe << "\n";
            }
            else std::cout << "no fruit detected \n";
        }
        ...
        auto camRgb = pipeline.create<dai::node::Camera>()->build(dai::CameraBoardSocket::CAM_A,std::nullopt,10);
        auto monoLeft = pipeline.create<dai::node::Camera>()->build(dai::CameraBoardSocket::CAM_B,std::nullopt,10);
        auto monoRight = pipeline.create<dai::node::Camera>()->build(dai::CameraBoardSocket::CAM_C,std::nullopt,10);
        // Create stereo node
        ...
        dai::NNArchive model(nnPath);
        //auto shavedBlob = model.getSuperBlob()->getBlobWithNumShaves(6);
        auto spatialDetectionNetwork = pipeline.create<dai::node::SpatialDetectionNetwork>()->build(camRgb, stereo, model);
        ...
        objectTracker->setDetectionLabelsToTrack({ 0,1 });  // track only person
